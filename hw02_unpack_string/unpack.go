...
func Unpack(input string) (string, error) {
	...
	for _, r := range input {
		...
		if !shielding && isDigit {
			...
			// цифрой и строка не может начинаться с цифры.
			if lastSimbol == "" {
				return "", ErrInvalidString
			}
			...
			lastSimbol = "" // Последний символ является экранированной цифрой.
		} else {
	...
	// предыдущем шаге, что влечет необходимость добавления последнего символа к
	// выходному буферу.
	b.WriteString(lastSimbol)
